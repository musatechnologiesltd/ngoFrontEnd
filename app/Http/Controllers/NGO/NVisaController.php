...
class NVisaController extends Controller
{
    ...
    public function store(Request $request){
        ...
        $fdOneFormId = FdOneForm::where('user_id',Auth::user()->id)->first();
         ...
         $particularOfForeignIncumbnets->company_share = $request->company_share;
         $particularOfEmployee->save();
        ...
        if(empty($request->foreign_supporting_staff)){
         ...
         if ($request->hasfile('foreign_supporting_staff')) {
            $filePath="necessaryDocument";
            $file = $request->file('foreign_supporting_staff');
            $necessaryDocument->foreign_supporting_staff =CommonController::pdfUpload($request,$file,$filePath);
        ...
        return redirect('/fdNineForm/create')->with('success','Created Successfully');

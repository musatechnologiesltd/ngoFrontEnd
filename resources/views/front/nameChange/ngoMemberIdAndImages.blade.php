                            ...
                            <div class="file-content">
                                <div class="card">
                                    <div class="card-body file-manager">
                                        <div class="files">
                                            @foreach($formEightList as $allAllNgoMemberDoc)


                                 <?php

$filePath = url($allAllNgoMemberDoc->member_nid_copy);
$filename  = pathinfo($filePath, PATHINFO_FILENAME);
                                ...
                                ?>
                                    ...
                                    <div class="file-box-image">
                                        <div class="file-top">
                                            <img class="file-top-img" src="{{ asset('/') }}{{ $allAllNgoMemberDoc->member_image }}"
                                                 alt="Card image cap">
                                        </div>
                                        <div class="mt-2">
                                            <h6>{{ $allAllNgoMemberDoc->member_name  }}</h6>
                                            <p class="mb-1">{{ $filename }}</p>
                                            <p>{{ $allAllNgoMemberDoc->member_nid_copy_size }} {{ trans('other_doc.m_b')}}</p>


                                            <a class="btn btn-sm btn-registration" target="_blank" href = '{{ route('ngoMemberDocumentDownload',$allAllNgoMemberDoc->id) }}'><i class="fa fa-download"></i>
                                            </a>

                                            <button class="btn btn-sm btn-primary" data-bs-toggle="modal"
                                            data-bs-target="#exampleModal{{ $allAllNgoMemberDoc->id  }}"><i class="fa fa-pencil"></i></button>

                                            <button class="btn btn-sm btn-danger" onclick="deleteTag({{ $allAllNgoMemberDoc->id}})"><i class="fa fa-trash"></i></button>

                                            <form id="delete-form-{{ $allAllNgoMemberDoc->id }}" action="{{ route('ngoMemberDocument.destroy',$allAllNgoMemberDoc->id) }}" method="POST" style="display: none;">
                            ...
                            <!--ddd-->
                            <div class="modal fade" id="exampleModal{{ $allAllNgoMemberDoc->id  }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        ...
                                        <div class="modal-body">
                                            <form method="post" action="{{ route('ngoMemberNidAndImageUpdate') }}" enctype="multipart/form-data">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <input type="text" name="person_name" value="{{ $allAllNgoMemberDoc->member_name  }}" class="form-control" id="">

                                                    <input type="hidden" name="id" value="{{ $allAllNgoMemberDoc->id  }}" class="form-control" id="">
                                                </div>
                                                <div class="mb-3">
                                                    <label for="" class="form-label">{{ trans('ngo_member_doc.image')}}</label>
                                                    <input type="file" name="person_image" class="form-control" id="">

                                                    <img src="{{ asset('/') }}{{ $allAllNgoMemberDoc->member_image  }}" height="30" />
                                                </div>
                                                <div class="mb-3">
                                                    <label for="" class="form-label">{{ trans('ngo_member_doc.nid_copy')}}</label>
                                                    <input type="file" name="person_nid_copy" class="form-control" id="">

                                                    <iframe src="{{ asset('/') }}{{'public/'. $allAllNgoMemberDoc->member_nid_copy  }}"
